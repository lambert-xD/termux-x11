...
public class WaylandCmdEntryPoint extends ICmdEntryInterface.Stub {
    public static final String ACTION_START =
        "com.termux.x11.WaylandCmdEntryPoint.ACTION_START";
    static final Handler handler;
    public static Context ctx;
    private final Intent intent = createIntent();
    private String xstartupCommand = null;
    ...
    public static void main(String[] args) {
        android.util.Log.i("WaylandCmdEntryPoint", "commit " + BuildConfig.COMMIT);
        handler.post(() -> new WaylandCmdEntryPoint(args));
        Looper.loop();
    }

    WaylandCmdEntryPoint(String[] args) {
        // Parse -xstartup for later execution (PR 2)
        for (int i = 0; args != null && i < args.length; i++) {
            if ("-xstartup".equals(args[i]) && i + 1 < args.length) {
                xstartupCommand = args[i + 1];
                break;
            }
        }
        ...
        if (xstartupCommand != null) {
            new Thread(() -> {
                ...
                if (!connected()) {
                    Log.e("WaylandCmdEntryPoint", "Timed out waiting for Wayland compositor before -xstartup");
                    return;
                }
                executeXstartup();
            }, "WaylandXstartupWaiter").start();
        }
    }

    private void executeXstartup() {
        if (xstartupCommand == null) return;
        try {
            ProcessBuilder pb = new ProcessBuilder("sh", "-c", xstartupCommand);
            pb.inheritIO();
            Map<String, String> env = pb.environment();
            String val;
            if ((val = getenv("XSTARTUP_LD_LIBRARY_PATH")) != null)
                env.put("LD_LIBRARY_PATH", val);
            if ((val = getenv("XSTARTUP_LD_PRELOAD")) != null)
                env.put("LD_PRELOAD", val);
            if ((val = getenv("XSTARTUP_CLASSPATH")) != null)
                env.put("CLASSPATH", val);
            Process process = pb.start();
            Log.i("WaylandCmdEntryPoint", "Started -xstartup: " + xstartupCommand);
            int exitCode = process.waitFor();
            Log.i("WaylandCmdEntryPoint", "-xstartup exited with code " + exitCode);
        } catch (Exception e) {
            Log.e("WaylandCmdEntryPoint", "Failed to execute -xstartup", e);
        }
    }
    ...
    static {
        try {
            ...
            Log.e("WaylandCmdEntryPoint", "Something went wrong when preparing MainLooper", e);
        }
        handler = new Handler();
        ctx = createContext();

        String path = "lib/" + Build.SUPPORTED_ABIS[0] + "/libXlorie.so";
        ClassLoader loader = WaylandCmdEntryPoint.class.getClassLoader();
        ...
        if (libPath != null) {
            try {
                ...
                Log.e("WaylandCmdEntryPoint", "Failed to dlopen " + libPath, e);
