...
#define PORT 7893  // Different port from X11 (7892)
#define MAGIC "0xWAYLAND"

// Reuse the event types and structures from lorie.h
// but without X11 dependencies
...
struct lorie_shared_server_state {
    /*
     * Renderer and compositor are separated into 2 different processes.
     * Root window and cursor content and properties are shared across these 2 processes.
     * Reading/drawing root window in renderer the same time compositor writes it can cause
     * tearing, texture garbling and other visual artifacts so we should block compositor while we are drawing.
     */
    pthread_mutex_t lock; // initialized at compositor side.
    pid_t lockingPid;

    /*
     * Renderer thread sleeps when it is idle so we must explicitly wake it up.
     */
    pthread_cond_t cond; // initialized at compositor side.
    ...
    /*
     * We do not want to block the compositor for an extended period; ideally, we would avoid blocking it at all.
     * However, if we don't block the compositor, it will overwrite root window memory fragment, causing tearing or frame distortion.
...
// Forward declarations for functions used by mutex helpers
bool lorieConnectionAlive(void);

// Mutex helpers (copied from lorie.h)
...
// Forward declarations for renderer functions (from renderer.c)
...
// Forward declarations for buffer functions (from buffer.c)
int ancil_send_fd(int sock, int fd);
int ancil_recv_fd(int sock);

// Keycode mapping (copied from lorie.h)
